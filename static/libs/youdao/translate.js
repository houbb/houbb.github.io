...
(function () {
    var a = {
        createNode: function (d, c, f) {
            ...
            if (a.isDOM(f)) {
                var d = f.className.split(" "), h = [], g = "add";
                for (var c = 0, b = d.length; c < b; c++) {
                    if (e === d[c]) {
                        g = "remove"
                    } else {
...
(function (b) {
    function a() {
        ...
        this.title = "on";
        this.resize = "on";
        this.relatedUrl = b.url();
        var d = {origin: 0, full: 0, half: 0, query: 0, button: 64, result: 0};
        var e = {origin: 0, full: 0, half: 0, top: 20, center: 0, query: 27, button: 23, result: 0};
    ...
    window.YoudaoSelector.Config = new a()
})(YoudaoUtils);
(function (c, a) {
    function b() {
        var h = this;
        ...
        this.init = function (o) {
            var j = this;
            i = document.getElementById("YOUDAO_SELECTOR_WRAPPER");
            a.init(i.style.width, i.style.height);
            var k = i.getAttribute("bindTo");
            if (k !== null) {
                a.bindTo = k
            }
            var n = i.getAttribute("borderColor");
            var m = i.getAttribute("backgroundImage");
            if (n !== null) {
                a.borderColor = n
            }
            if (m !== null) {
            ...
            setTimeout(function () {
                try {
                    j.iframeDocument = l.contentDocument || l.contentWindow.document
                } catch (p) {
                    alert("由于该网页存在安全性限制, 无法加载有道翻译");
                    return
                }
                g(j.iframeDocument);
                j.resultDiv = f(j.iframeDocument);
                d(j.iframeDocument, true);
                o.call()
            }, 100)
        };
        function e() {
            i.innerHTML = '<iframe id="YOUDAO_SELECTOR_IFRAME" frameBorder="0" src="about:blank"></iframe>';
            var j = document.getElementById("YOUDAO_SELECTOR_IFRAME");
            c.css(j, {
                "background-color": "white",
                border: "0",
                width: a.getWidth("full") + "px",
                height: a.getHeight("full") + "px"
            });
            if (c.browser.msie) {
                if (document.domain != document.location.hostname) {
                    j.src = "<script>document.domain='" + document.domain + "'<\/script>"
                }
            }
            return j
        }
        ...
        function f(m) {
            ...
            if (c.browser.msie) {
                c.loadCSSToLink(m, "linkStyle", k)
            } else {
                c.loadCSS(m, k)
            }
            var l = null;
            if (a.backgroundImage !== null) {
                l = '<div id="youdaoDictBg"style="background: url(' + a.backgroundImage + ") repeat 0 0; width:" + a.getWidth("full") + "px; height:" + a.getHeight("full") + 'px;"></div>'
            } else {
            ...
            m.body.innerHTML = l + '                <div id="youdaoDictMain">' + j + '                    <div id="youdaoDictCenter">                        <div id="result" class="no-x-scroll"></div>                    </div>                </div>';
            return m.getElementById("result")
        }

        function d(m, n) {
            if (n === false) {
                c.css(i, {width: a.getWidth("full") + "px", height: a.getHeight("full") + "px"});
                c.css(document.getElementById("YOUDAO_SELECTOR_IFRAME"), {
                    width: a.getWidth("full") + "px",
                    height: a.getHeight("full") + "px"
                })
            }
            var j = m.getElementById("youdaoDictBg");
            c.css(j, {width: a.getWidth("full") + "px", height: a.getHeight("full") + "px"});
            var l = m.getElementById("bgTop");
            if (l !== null) {
                c.css(l, {height: a.getHeight("half") + "px"});
                c.css(m.getElementById("bgTopRight"), {
                    width: a.getWidth("half") + "px",
                    height: a.getHeight("half") + "px"
                });
                c.css(m.getElementById("bgBottom"), {height: (a.getHeight("full") - a.getHeight("half")) + "px"});
                c.css(m.getElementById("bgBottomRight"), {
                    width: a.getWidth("half") + "px",
                    height: (a.getHeight("full") - a.getHeight("half")) + "px"
                })
            }
            c.css(m.getElementById("youdaoDictMain"), {height: a.getHeight("full") + "px"});
            var k = m.getElementById("youdaoDictTop");
            if (k !== null) {
                c.css(m.getElementById("youdaoDictTop"), {height: a.getHeight("top") + "px"})
            }
            c.css(m.getElementById("youdaoDictCenter"), {height: a.getHeight("center") + "px"});
            c.css(m.getElementById("result"), {
                "border-color": a.borderColor,
                width: a.getWidth("result") + "px",
                height: a.getHeight("result") + "px"
            })
        }

        this.getSelectEventPos = function (k, l) {
            var r = this;
        ...
        this.getSelectorPos = function (o) {
            ...
            var m = c.scroll().left;
            var l = c.scroll().top;
            var k = c.windowSize().width;
            if (j + 10 + a.getWidth("full") + 20 <= k + m) {
                j += 10
            } else {
                j = k + m - a.getWidth("full") - 20
            }
            if (n - 20 - a.getHeight("full") - 20 >= l) {
                n = n - 20 - a.getHeight("full")
            } else {
                n += 20
            }
            o.x = j;
        ...
        this.resize = function () {
            ...
            var v = w + y + l + p + x + q + s;
            if (a.reConfigByResultHeight(v)) {
                d(m, false)
            }
        };
        this.show = function (j) {
            c.css(i, {display: "block", position: "absolute", left: j.x + "px", top: j.y + "px"})
        };
        this.hide = function () {
            c.css(i, {display: "none"})
        }
    }

    window.YoudaoSelector.UI = new b()
...
(function (b) {
    ...
    window.YoudaoSelector.DataRender = a
})(YoudaoUtils);
(function (e, a, d, c) {
    function b() {
        var s = this;
        ...
        function h() {
            ...
            f = d.resultDiv;
            if (a.bindTo !== null) {
                var w = a.bindTo.split(":");
                for (var x = 0, v = w.length; x < v; x++) {
                    e.bind(document.getElementById(w[x]), "mouseup", n)
                }
            } else {
                e.bind(document.body, "mouseup", n)
            }
            e.bind(document.body, "mousedown", d.hide);
            g()
        }
        ...
        function g() {
            ...
            if (v !== null) {
                ...
                a.select = "off";
                e.bind(v, "click", function () {
                    var w = v.getAttribute("status");
                    if (w === "on") {
                        e.css(v, {backgroundPosition: "0 0"});
                        v.setAttribute("status", "off");
                        v.setAttribute("title", j);
                        a.select = "off"
                    } else {
                        e.css(v, {backgroundPosition: "-20px 0"});
        ...
        this.update = function (v) {
            ...
            if (v.errorCode !== 0) {
                ...
                if (v.basic === undefined && v.web === undefined) {
                    c.renderTranslation(l, f, v)
                } else {
                    if (a.title === "off") {
                        c.renderCopyRight(l, f)
                    }
                    c.renderTitle(l, f, v);
                    if (v.basic !== undefined) {
                        c.renderBasicResult(l, f, v)
                    }
                    if (v.web !== undefined) {
                        c.renderWebResult(l, f, v)
                    }
                    c.renderMore(l, v)
                }
            }
            c.renderBottom(l, f, v);
            d.tryResize(v);
            w = d.getSelectorPos(w);
            d.show(w);
            d.resize();
            f.scrollTop = 0
        };
        ...
        function t(w) {
            ...
            x.setAttribute("src", "http://fanyi.youdao.com/openapi.do?type=data&doctype=jsonp&version=1.1&" + e.parameter({
                    relatedUrl: a.relatedUrl,
        ...
        function q(w) {
            ...
            var x = document.createElement("script");
            x.setAttribute("type", "text/javascript");
            x.setAttribute("src", "http://fanyi.youdao.com/openapi.do?type=data&only=on&doctype=jsonp&version=1.1&" + e.parameter({
                    relatedUrl: a.relatedUrl,
                    keyfrom: "20170413PersonApi",
                    key: "477661285",
                    callback: y,
    ...
    window.YoudaoSelector.Instance = new b()
