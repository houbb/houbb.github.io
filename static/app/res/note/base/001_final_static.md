...
1. As usual, 对象 在 new 之后存储空间才会被分配，方法才会被外部调用。
 
2. There are two case that new do not work well
 
1) 特定的数据分配一份存储空间，无论多少对象。或者不需要新建对象
 
2) 希望方法不与包含他的类的任何对象关联在一起。
...
public int instanceVar = 0; public VariantTest(){
instanceVar); }
}
```
