...
➜ go get github.com/golangci/golangci-lint/cmd/[[email protected]](https://learn.lianglianglee.com/cdn-cgi/l/email-protection)

使用这一命令安装的是 v1.32.2 版本的 golangci-lint，安装完成后，在终端输入如下命令，检测是否安装成功。

➜ golangci-lint version golangci-lint has version v1.32.2
...
ch19/main.go:5:7: `name` is unused (deadcode) const name = "飞雪无情" ^ ch19/main.go:8:10: Error return value of `os.Mkdir` is not checked (errcheck) os.Mkdir("tmp",0666)
