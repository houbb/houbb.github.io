---
layout: post
title:  22讲通关Go语言~完-07错误处理：如何通过error、deferred、panic等处理错误？
...
type WalkRun interface { Walk() Run() }
...
***ch07/main.go***
func add(a,b int) (int,error){ if a<0 || b<0 { return 0,errors.New("a或者b不能为负数") }else { return a+b,nil } }
...
***ch07/main.go***
sum,err:=add(-1,2) if err!=nil { fmt.Println(err) }else { fmt.Println(sum) }
...
***ch07/main.go***
type commonError struct { errorCode int //错误码 errorMsg string //错误信息 } func (ce /*commonError) Error() string{ return ce.errorMsg }
...
***ch07/main.go***
sum, err := add(-1, 2) if cm,ok:=err.(/*commonError);ok{ fmt.Println("错误代码为:",cm.errorCode,"，错误信息为：",cm.errorMsg) } else { fmt.Println(sum) }
...
type MyError struct { err error msg string }
...
func (e /*MyError) Error() string { return e.err.Error() + e.msg } func main() { //err是一个存在的错误，可以从另外一个函数返回 newErr := MyError{err, "数据上传问题"} }
...
***ch07/main.go***
e := errors.New("原始错误e") w := fmt.Errorf("Wrap了一个错误:%w", e) fmt.Println(w)
...
***ch07/main.go***
var cm /*commonError if errors.As(err,&cm){ fmt.Println("错误代码为:",cm.errorCode,"，错误信息为：",cm.errorMsg) } else { fmt.Println(sum) }
...
func ReadFile(filename string) ([]byte, error) { f, err := os.Open(filename) if err != nil { return nil, err } defer f.Close() //省略无关代码 return readAll(f, n) }
...
![pDE7ppQNyfRSIn1Q__thumbnail.png](https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/22%20%e8%ae%b2%e9%80%9a%e5%85%b3%20Go%20%e8%af%ad%e8%a8%80-%e5%ae%8c/assets/CgqCHl-15ZSAAsw5AAUnpsfN34w061.png)
