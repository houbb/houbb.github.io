...
> 范围：仅设计方案 + 交互说明 + 数据模型 + API 划分，不包含具体业务代码实现。
...
```
输入邮箱/用户名 → 输入密码 → 实时检测 → 提交 → 二次校验 → 创建账号
```
...
为 Passport 从“基础账号系统”升级为“安全中枢”奠定基础。
