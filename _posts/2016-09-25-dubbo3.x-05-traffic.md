...
> 这里牵涉到如何给您的实例打各种原始 label 的问题，即上面示例中的 `env`，一种方式是直接写在配置文件中，如上面静态规则实例 provider 部分的配置所示，另一种方式是通过预设环境变量指定，关于这点请参考下文的 [如何给实例打标](/zh-cn/overview/what/core-features/traffic/#如何给实例打标) 一节。
...
> 针对条件路由，我们通常推荐配置 `scope: service` 的规则，因为它可以跨消费端应用对所有消费特定服务 (service) 的应用生效。关于 Dubbo 规则中的 `scope` 以及 `service`、`application` 的说明请阅读 [条件路由规则手册](/zh-cn/overview/what/core-features/traffic/condition-rule/)。
...
> 关于 Dubbo 规则中的 `scope` 以及 `service`、`application` 的说明请参考 [动态配置参考手册](/zh-cn/overview/what/core-features/traffic/configuration-rule/) 或 [动态配置示例](../../tasks/traffic-management/timeout/)。
...
具体请参见 [Dubbo 中的服务网格架构](/zh-cn/overview/what/core-features/service-mesh/)。
