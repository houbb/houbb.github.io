...
java 5中最好的增加之一是在类中支持的原子操作，比如 AtomicInteger、AtomicLong 等等。

这些类内部依赖于一个名为CAS的算法(比较和交换)。在本文中，我将详细讨论这个概念。
...
有一种乐观锁的实现方式就是CAS，这种算法在 JDK 1.5 中引入的 `java.util.concurrent` 中有广泛应用。

但是值得注意的是这种算法会存在ABA问题。
