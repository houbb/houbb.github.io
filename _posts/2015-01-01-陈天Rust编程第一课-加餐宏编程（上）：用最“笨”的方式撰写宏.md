...
match result { Ok(v) => v, Err(e) => { return pipeline::PlugResult::Err { ctx, err: pipeline::PipelineError::Internal(e.to_string()), } } }

但是这种写法，在同一个函数内，可能会反复出现，我们又无法用函数将其封装，所以我们可以用声明宏来实现，可以大大简化代码：

/#[macro_export] macro_rules! try_with { ($ctx:ident, $exp:expr) => { match $exp { Ok(v) => v, Err(e) => { return pipeline::PlugResult::Err { ctx: $ctx, err: pipeline::PipelineError::Internal(e.to_string()), } } } }; }
...
use proc_macro::TokenStream; /#[proc_macro] pub fn query(input: TokenStream) -> TokenStream { println!("{:/#?}", input); "fn hello() { println!(\\"Hello world!\\"); }" .parse() .unwrap() }
...
{} [] <> ()
，那么内部的内容会被分析成一个 Group（组）。你也可以试试把例子中对
...
/#[derive(Debug, Default)] struct Fd { name: String, ty: String, optional: bool, }
...
/#[proc_macro_derive(RawBuilder)] pub fn derive_raw_builder(input: TokenStream) -> TokenStream { println!("{:/#?}", input); TokenStream::default() }
...
use macros::RawBuilder; /#[allow(dead_code)] /#[derive(Debug, RawBuilder)] pub struct Command { ... }
