...
#### 1. 事件表示与排序
假设我们有若干个区间（如会议的开始时间和结束时间），我们首先将每个区间拆解为两个事件：一个是开始事件，另一个是结束事件。每个事件可以表示为一个元组 `(time, type)`，其中 `time` 表示事件发生的时间，`type` 可以是 `+1`（表示开始）或者 `-1`（表示结束）。
...
以下是 10 道在 LeetCode 中经典的扫描线算法题目，涵盖了区间覆盖、最大并发数、会议室分配等不同类型的应用场景。

这些题目使用扫描线算法能够高效地解决，因为扫描线算法善于处理包含时间段、区间重叠、覆盖等的复杂问题。

### 1. [Meeting Rooms II (LC 253)](https://leetcode.com/problems/meeting-rooms-ii/)
   - **描述**：给定一个会议时间安排的数组，计算需要的最小会议室数量。
   - **解法**：使用扫描线来统计同时进行的会议数，求出同时进行的最大会议数。

### 2. [The Skyline Problem (LC 218)](https://leetcode.com/problems/the-skyline-problem/)
   - **描述**：给定一组建筑，计算城市天际线的轮廓。
   - **解法**：将建筑的左右边界作为事件，使用扫描线计算出天际线的变化位置。

### 3. [Employee Free Time (LC 759)](https://leetcode.com/problems/employee-free-time/)
   - **描述**：给定员工的忙碌时间表，找到所有员工的空闲时间段。
   - **解法**：将所有时间段作为事件进行排序并使用扫描线算法，以识别重叠区域和空闲区间。

### 4. [Car Pooling (LC 1094)](https://leetcode.com/problems/car-pooling/)
   - **描述**：给定一组乘车请求（上车点、下车点和人数），检查是否能满足所有乘客的请求。
   - **解法**：将上车和下车点作为事件进行排序，通过扫描线统计每时刻的乘客数，确保没有超过车的容量。

### 5. [Merge Intervals (LC 56)](https://leetcode.com/problems/merge-intervals/)
   - **描述**：给定一组区间，将重叠的区间合并。
   - **解法**：扫描线算法可以用于检测区间的重叠并合并重叠区间。

### 6. [Insert Interval (LC 57)](https://leetcode.com/problems/insert-interval/)
   - **描述**：给定一个区间列表和一个新区间，插入并合并可能的重叠区间。
   - **解法**：将新区间和已有区间作为事件，使用扫描线处理重叠并进行合并。

### 7. [Minimum Number of Arrows to Burst Balloons (LC 452)](https://leetcode.com/problems/minimum-number-of-arrows-to-burst-balloons/)
   - **描述**：给定气球的位置，计算需要的最少箭数，使得每个气球都至少被一个箭射中。
   - **解法**：将每个气球的位置视为区间，使用扫描线找出所有重叠气球的区间，并确定箭的数量。

### 8. [My Calendar I (LC 729)](https://leetcode.com/problems/my-calendar-i/)
   - **描述**：设计一个日历，支持不重叠的会议预定。
   - **解法**：将每个预定的开始时间和结束时间作为事件，使用扫描线判断是否有重叠。

### 9. [My Calendar II (LC 731)](https://leetcode.com/problems/my-calendar-ii/)
   - **描述**：设计一个日历，支持双重预定（最多允许两个重叠）。
   - **解法**：使用扫描线算法，允许最多两个同时发生的事件，来确定是否符合要求。

### 10. [My Calendar III (LC 732)](https://leetcode.com/problems/my-calendar-iii/)
   - **描述**：设计一个日历，返回任意时刻的最大重叠数。
   - **解法**：使用扫描线算法，求出所有时间点上重叠的最大事件数量。

这些题目都可以利用扫描线算法的优势来解决，通过事件的增减与排序，可以高效地处理区间重叠和活动统计。
