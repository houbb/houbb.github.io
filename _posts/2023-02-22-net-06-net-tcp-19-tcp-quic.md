---
layout: post
title: NET 网络专题汇总-06-19-如何基于 UDP 协议实现可靠传输？
...
在之前这篇文章：TCP 就没什么缺陷吗？ (opens new window)，我已经说了 TCP 协议四个方面的缺陷：
...
![QUIC](https://cdn.xiaolincoding.com//mysql/other/605d1026df934f20a5ee12f3c55aa6a7.png)
...
![TLS1.3](https://cdn.xiaolincoding.com//mysql/other/ab3283383013b707d1420b6b4cb8517c.png)

整体看的视角是这样的：

![UDP d](https://cdn.xiaolincoding.com//mysql/other/http3-over-quic-protocol-works.png)
...
如下图（注意我没有把 Header 所有字段都画出来，只是画出了重要的字段）：
...
![重传](https://cdn.xiaolincoding.com//mysql/other/635813465fbb449882da2e2bee39f24e.png)

## 为什么要这么设计呢？

我们先来看看 TCP 的问题，TCP 在重传报文时的序列号和原始报文的序列号是一样的，也正是由于这个特性，引入了 TCP 重传的歧义问题。

![why](https://cdn.xiaolincoding.com/gh/xiaolincoder/network/quic/tcp%E9%87%8D%E4%BC%A0%E6%AD%A7%E4%B9%89.jpeg)
...
![重传](https://cdn.xiaolincoding.com//mysql/other/ca91985c9a94487a8a29db1249109717.png)
...
![Header](https://cdn.xiaolincoding.com//mysql/other/6a94d41ef3d14cb6b7846e73da6c3104.png)

每一个 Frame 都有明确的类型，针对类型的不同，功能也不同，自然格式也不同。

我这里只举例 Stream 类型的 Frame 格式，Stream 可以认为就是一条 HTTP 请求，它长这样：

![stream](https://cdn.xiaolincoding.com//mysql/other/536298d2c54a43b699026bffe0f85010.png)
...
![包丢失](https://cdn.xiaolincoding.com/gh/xiaolincoder/network/quic/Packet%E4%B8%A2%E5%A4%B1.jpeg)
...
![接收](https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost2/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C/TCP-%E5%8F%AF%E9%9D%A0%E7%89%B9%E6%80%A7/20.jpg)
...
![对头阻塞](https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost4@main/%E7%BD%91%E7%BB%9C/http2/stream2.png)
...
![读取](https://cdn.xiaolincoding.com/gh/xiaolincoder/network/quic/http2%E9%98%BB%E5%A1%9E.jpeg)
...
![QUIC](https://cdn.xiaolincoding.com/gh/xiaolincoder/network/quic/quic%E6%97%A0%E9%98%BB%E5%A1%9E.jpeg)
...
而对于 HTTP/2 而言，所有的 Stream 都跑在一条 TCP 连接上，而这些 Stream 共享一个滑动窗口，因此同一个Connection内，Stream A 被阻塞后，StreamB、C 必须等待。
...
![流控](https://cdn.xiaolincoding.com//mysql/other/f1070a6eccd24559904815297b07f789.png)

接着，接收方收到了发送方发送过来的数据，有的数据被上层读取了，有的数据丢包了，此时的接收窗口状况如下：

![最大接收窗口](https://cdn.xiaolincoding.com//mysql/other/77e9a7cf70da4a1b981f61e78db2ad56.png)
...
![右边界](https://cdn.xiaolincoding.com//mysql/other/bbde0c66088f439b919a6d18b389aadb.png)
...
![乱序确认](https://cdn.xiaolincoding.com/gh/xiaolincoder/network/quic/%E4%B9%B1%E5%BA%8F%E7%A1%AE%E8%AE%A41.png)

31、32、34数据包先到达，基于 offset 被优先乱序确认，但 30 数据包没有确认，所以当前已提交的字节偏移量不变，发送方的缓存区不变。

![乱序确认-2](https://cdn.xiaolincoding.com/gh/xiaolincoder/network/quic/%E4%B9%B1%E5%BA%8F%E7%A1%AE%E8%AE%A42.png)

30 到达并确认，发送方的缓存区收缩到阈值，接收方发送 MAX_STREAM_DATA Frame（协商缓存大小的特定帧）给发送方，请求增长最大绝对字节偏移量。

![乱序确认-3](https://cdn.xiaolincoding.com/gh/xiaolincoder/network/quic/%E4%B9%B1%E5%BA%8F%E7%A1%AE%E8%AE%A43.png)

协商完毕后最大绝对字节偏移量右移，发送方的缓存区变大，同时发送方发现数据包33超时

![乱序确认-4](https://cdn.xiaolincoding.com/gh/xiaolincoder/network/quic/%E4%B9%B1%E5%BA%8F%E7%A1%AE%E8%AE%A44.png)

发送方将超时数据包重新编号为 42 继续发送

![乱序确认-5](https://cdn.xiaolincoding.com/gh/xiaolincoder/network/quic/%E4%B9%B1%E5%BA%8F%E7%A1%AE%E8%AE%A45.png)
...
![Q](https://cdn.xiaolincoding.com//mysql/other/839501cffa7146cbb8d992264594e61d.png)
...
这是一个飞跃，因为传统的 TCP 拥塞控制，必须要端到端的网络协议栈支持，才能实现控制效果。
...
但是因为 QUIC 处于应用层，所以就可以针对不同的应用设置不同的拥塞控制算法，这样灵活性就很高了。
...
![更快的连接建立](https://cdn.xiaolincoding.com//mysql/other/4cad213f5125432693e0e2a512c2d1a1.png)
