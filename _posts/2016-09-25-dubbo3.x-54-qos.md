---
layout: post
title: Dubbo3.x-54-单机运维命令 QOS 概述
...
# QoS 命令的设计目的、使用方法说明，包括如何开启、关闭 qos 命令等，支持 HTTP/Telnet 访问方式
...
telnet 模块现在同时支持 http 协议和 telnet 协议，方便各种情况的使用 示例：
...
> 权限级别 `PRIVATE`> `PROTECTED`> `PUBLIC`> `NONE`, 高级别权限可访问同级别和低级别权限命令。 当前以下命令权限为`PUBLIC`, 其它命令默认权限别为`PROTECTED`。
