...
我们再来看 Go 语言这个模式，Go 语言的这个模式挺好玩儿的。
...
但是，需要注意的是，我们用了一个新的 UndoableIntSe 几乎重写了所有的 IntSet
...
我们再次看到，Go 语言的 Undo 接口把 Undo 的流程给抽象出来，而要怎么 Undo 的事交给了业务代码来维护（通过注册一个 Undo 的方法）。
