---
layout: post
title: Dubbo3.x-60-单端口多协议实现原理解析
...
# Dubbo 单端口多协议实现原理&源码解析

通过对protocol进行配置，dubbo3可以支持端口的协议复用。 比如使用Triple协议启动端口复用后，可以在相同的端口上为服务增加 Dubbo协议支持，以及Qos协议支持。

这些协议的识别都是由一个统一的端口复用 服务器进行处理的，可以用于服务的协议迁移，并且可以节约端口以及相关的资源，减少运维的复杂性。

![pu-server-image1](https://cn.dubbo.apache.org/imgs/blog/pu-server/pu-server-flow.png)
...
> 参考用例 [https://github.com/apache/dubbo-samples/tree/master/dubbo-samples-port-unification](https://github.com/apache/dubbo-samples/tree/master/3-extensions/protocol/dubbo-samples-port-unification)
...
![pu-server-image2](https://cn.dubbo.apache.org/imgs/blog/pu-server/qos-telnet-directcall.png)
...
![pu-server-imgs3](https://cn.dubbo.apache.org/imgs/blog/pu-server/qos-telnet-sendayt.png)
...
![pu-server-imgs4](https://cn.dubbo.apache.org/imgs/blog/pu-server/reference-service.png)

# 参考资料

https://cn.dubbo.apache.org/zh-cn/overview/mannual/java-sdk/reference-manual/architecture/multi-protocol/
