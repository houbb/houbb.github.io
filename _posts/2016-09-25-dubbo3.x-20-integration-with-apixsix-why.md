---
layout: post
title: Dubbo3.x-22-生态集成之使用 Apache APISIX 代理 Dubbo 服务 (dubbo 协议)
...
[Apache APISIX](https://apisix.apache.org/) 是 Apache 软件基金会的顶级开源项目，也是当前最活跃的开源网关项目。

作为一个动态、实时、高性能的开源 API 网关，Apache APISIX 提供了负载均衡、动态上游、灰度发布、服务熔断、身份认证、可观测性等丰富的流量管理功能。
...
![架构图](https://cn.dubbo.apache.org/imgs/blog/apisix-plugin/1.png)
...
discovery:
  nacos:
host:
- "[http://192.168.33.1:8848 ](http://192.168.33.1:8848 )"
```
...
```bash
$ git clone -b master --depth 1 [https://github.com/apache/dubbo-samples ](https://github.com/apache/dubbo-samples )
...
```bash
curl [http://127.0.0.1:9080/demo ](http://127.0.0.1:9080/demo )  -H "Host: example.org "  -X POST --data '{"name": "hello"}'
...
:::note 说明 上述代码返回中包含了 `test: 123` Header，以及 `dubbo success` 字符串作为 Body 体。这与我们在 `DemoServiceImpl` 编码的预期效果一致。 :::
...
Key = content-type, Value = application/x-[www-form-urlencoded](https://www-form-urlencoded)
Key = body, Value = [B@70754265
Key = accept, Value = */*
Key = user-agent, Value = curl/7.80.0
```

:::note 说明 通过 `httpRequestContext` 可以拿到 HTTP 请求的 Header 和 Body。其中 Header 会作为 Map 元素，而 Body 中 Key 值是固定的字符串"body"，Value 则代表 Byte 数组。 :::
...
```bash
curl [http://127.0.0.1:9080/demo ](http://127.0.0.1:9080/demo )  -H "Host: example.org "  -X POST --data '
{
...
关于这部分的更多示例，还可以参考 [https://github.com/chickenlj/APISIX-Dubbo-Nacos](https://github.com/chickenlj/APISIX-Dubbo-Nacos)
```
...
https://cn.dubbo.apache.org/zh-cn/blog/2024/04/25/%E4%BD%BF%E7%94%A8-apache-apisix-%E4%BB%A3%E7%90%86-dubbo-%E6%9C%8D%E5%8A%A1-dubbo-%E5%8D%8F%E8%AE%AE/  原封不动 markdown 返回给我,代码、命令、配置等信息，加上合适的代码块。配置要保持格式正确，特别是 yaml
