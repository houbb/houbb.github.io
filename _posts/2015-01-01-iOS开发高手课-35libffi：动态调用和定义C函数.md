---
layout: post
title:  iOS开发高手课-35libffi：动态调用和定义C函数
...
typedef struct _ffi_type { size_t size; // 所占大小 unsigned short alignment; //对齐大小 unsigned short type; // 标记类型的数字 struct _ffi_type /*/*elements; // 结构体中的元素 } ffi_type;
...
typedef struct { ffi_abi abi; // 不同 CPU 架构下的 ABI，一般设置为 FFI_DEFAULT_ABI unsigned nargs; // 参数个数 ffi_type /*/*arg_types; // 参数类型 ffi_type /*rtype; // 返回值类型 unsigned bytes; // 参数所占空间大小，16的倍数 unsigned flags; // 返回类型是结构体时要做的标记 /#ifdef FFI_EXTRA_CIF_FIELDS FFI_EXTRA_CIF_FIELDS; /#endif } ffi_cif;
...
extern void ffi_call_SYSV (void /*stack, void /*frame, void (/*fn)(void), void /*rvalue, int flags, void /*closure);
...
// 计算矩形面积 int rectangleArea(int length, int width) { printf("Rectangle length is %d, and with is %d, so area is %d \n", length, width, length /* width); return length /* width; } void run() { // dlsym 返回 rectangleArea 函数指针 void /*dlsymFuncPtr = dlsym(RTLD_DEFAULT, "rectangleArea"); }
...
https://learn.lianglianglee.com/%e4%b8%93%e6%a0%8f/iOS%e5%bc%80%e5%8f%91%e9%ab%98%e6%89%8b%e8%af%be/35%20libffi%ef%bc%9a%e5%8a%a8%e6%80%81%e8%b0%83%e7%94%a8%e5%92%8c%e5%ae%9a%e4%b9%89%20C%20%e5%87%bd%e6%95%b0.md
