...
> [更多信息请见以太坊黄皮书（附录 B）](https://ethereum.github.io/yellowpaper/paper.pdf#page=19)。

要使用递归长度前缀对字典进行编码，建议的两种规范形式为：

1. 使用 [[k1,v1],[k2,v2]...] 加上按字典顺序排列的键

2. 像以太坊一样使用更高级别的前缀树编码
...
3 的集合论表示，[ [], [[]], [ [], [[]] ] ] = [ 0xc7, 0xc0, 0xc1, 0xc0, 0xc3, 0xc0, 0xc1, 0xc0 ]

字符串“Lorem ipsum dolor sit amet, consectetur adipisicing elit”= [ 0xb8, 0x38, 'L', 'o', 'r', 'e', 'm', ' ', ... , 'e', 'l', 'i', 't' ]

# 递归长度前缀解码

根据递归长度前缀编码的规则和过程，递归长度前缀译码的输入被视为一个二进制数据数组。 

递归长度前缀解码过程如下：

    根据输入数据的第一个字节（即前缀），解码数据类型、实际数据的长度和偏移量；

    根据数据类型和偏移量，对数据进行相应的解码；
