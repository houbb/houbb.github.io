...
sum := 0 for i:=1; i<100; i++{ if i%2!=0 { continue } sum+=i } fmt.Println("the sum is",sum)
...
***ch04/main.go***
nameAgeMap:=make(map[string]int) nameAgeMap["飞雪无情"] = 20 age,ok:=nameAgeMap["飞雪无情1"] if ok { fmt.Println(age) }
...
***ch04/main.go***
//测试 for range nameAgeMap["飞雪无情"] = 20 nameAgeMap["飞雪无情1"] = 21 nameAgeMap["飞雪无情2"] = 22 for k,v:=range nameAgeMap{ fmt.Println("Key is",k,",Value is",v) }
...
***ch04/main.go***
s:="Hello飞雪无情" bs:=[]byte(s)

string 不止可以直接转为 []byte，还可以使用 [] 操作符获取指定索引的字节值，如以下示例：

***ch04/main.go***
s:="Hello飞雪无情" bs:=[]byte(s) fmt.Println(bs) fmt.Println(s[0],s[1],s[15])
