...
经常被忽略的是，Vue 应用中响应式 data 对象的实际来源——当访问数据对象时，一个组件实例只是简单的代理访问。

所以，如果你有一处需要被多个实例间共享的状态，你可以使用一个 reactive 方法让对象作为响应式对象。
...
现在当 sourceOfTruth 发生变更，appA 和 appB 都将自动地更新它们的视图。

我们现在只有一个真实来源，但调试将是一场噩梦。

我们应用的任何部分都可以随时更改任何数据，而不会留下变更过的记录。
...
# 参考资料
https://vue3js.cn/docs/zh/guide/state-management.html
