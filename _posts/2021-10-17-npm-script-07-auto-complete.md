...
available via `npm run-script`:
  ...
  postcover
    npm-run-all cover:archive cover:cleanup --parallel cover:serve cover:open
(END)
```
...
npm 自身提供了自动完成工具 completion，将其集成到 bash 或者 zsh 里也非常容易（顺便说一句，早期我是 bash 的忠实用户，两年前切换到 zsh，就再也没回头）。
...
人类对于效率的追求是永无止境的，工程师更是如此，npm 命令补全到目前为止显然还不够高效，能不能补全 package.json 里面的依赖名称？

能不能在补全 npm script 的时候列出这个命令是干啥的？
...
看到这里，是不是心痒痒？具体的安装方法参照官方 README.md 文件就好，我就不在这里啰嗦了。
...
https://www.kancloud.cn/sllyli/npm-script/1243456
